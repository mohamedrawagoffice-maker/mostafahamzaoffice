...
// بيحسب لكل شخص: أول LEAVE_ANNUAL_PAID_DAYS يوم في السنة (بترتيب التاريخ) = براتب كامل، وأي يوم بعدها = خصم من الراتب
export function summarizeLeaveYear(leaves, userId, year, annualPaidDays) {
  const rows = leaves
    .filter((l) => l.user_id === userId && new Date(l.date).getFullYear() === year)
    .sort((a, b) => new Date(a.date) - new Date(b.date));
  const paid = rows.slice(0, annualPaidDays);
  const unpaid = rows.slice(annualPaidDays);
  const paidIds = new Set(paid.map((r) => r.id));
  return {
    total: rows.length,
    paidCount: paid.length,
    unpaidCount: unpaid.length,
    remaining: Math.max(0, annualPaidDays - paid.length),
