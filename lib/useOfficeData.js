...
export function useOfficeData(profile) {
  ...
  const updateTask = async (task, patch) => {
    ...
    const shortText = task.text.length > 60 ? task.text.slice(0, 60) + "…" : task.text;
    if (patch.status === "تم العلم" && task.from_user_id && task.from_user_id !== profile.id) {
      sendNotification({ userIds: [task.from_user_id], title: "تم العلم بالمهمة", body: `${profile.display_name} علّم بتنفيذ: ${shortText}`, url: "/tasks" });
    } else if (patch.status === "معتمد التنفيذ" && task.to_user_id && task.to_user_id !== profile.id) {
      sendNotification({ userIds: [task.to_user_id], title: "تم اعتماد تنفيذ مهمتك", body: shortText, url: "/tasks" });
    } else if ("note" in patch) {
      const others = [task.from_user_id, task.to_user_id].filter((id) => id && id !== profile.id);
      if (others.length > 0) {
        sendNotification({ userIds: others, title: "ملاحظة جديدة على مهمة", body: `${profile.display_name}: ${shortText}`, url: "/tasks" });
      }
    }
  };
  ...
  const notifyCustodyEvent = (entry) => {
    ...
    sendNotification({
      userIds: recipients, title: `حركة عهدة: ${label}`,
      body: `${profile.display_name} سجّل ${label} بمبلغ ${entry.amount} لـ${entry.user_name}`, url: "/custody",
    });
  };
